...
public class p1406 {
    public static void main(String[] args) throws IOException {
        ...
        String str = bf.readLine(); // 첫 줄 "abcd"
        int N = Integer.parseInt(bf.readLine()); // 둘째줄 "3"

        List<Character> list = new LinkedList<>();

        for (char c : str.toCharArray()) list.add(c);// string을 list에 넣음

        ListIterator<Character> listIterator = list.listIterator(list.size()); // list를 Iterator로 바꿈

        for(int i = 0; i < N; i++) {
            StringTokenizer st = new StringTokenizer(bf.readLine()); // 셋째 ~ 마지막줄 cmd
